...
D=${1:?"usage: $0 <d>   (e.g. 2, 3, 4)"}

CFG=configs/sweep-intermediate/${D}/config.json
if [ ! -f "$CFG" ]; then
    echo "config not found: $CFG" >&2; exit 1
fi

DATA_DIR=./dataset
OUT=logs/sweep-pets-intermediate-fp32-${D}

torchrun --nproc_per_node=1 --master_port=25641 main_new.py \
    --resume=logs/sweep-pets-intermediate-fp32-4/checkpoint.pth \
    --num-workers=4 \
    --batch-size=24 \
    --epochs=1000 \
    --dropout=0.0 \
