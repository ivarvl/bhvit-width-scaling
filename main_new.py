...
def get_args_parser():
    ...
    parser.add_argument(
        ...
        default=0.1,
        type=float,
        help="fraction of the training set held out (uniformly per class) as the "
        "validation set used for model selection. The test set is never used here; "
        "evaluate on it with evaluate_test.py.",
    )
    parser.add_argument(
        "--inat-category",
        default="name",
    ...
    parser.add_argument(
        "--init-from",
        default="",
        help="warm-start model weights from a (e.g. FP) checkpoint via a "
        "shape-tolerant partial load. Only tensors whose name and shape match "
        "are copied; the rest keep their initialization. Affects weights only "
        "(no optimizer/epoch state); ignored when --resume is set.",
    )
    parser.add_argument(
        "--current-best-model",
        default="",
...
def main(args):
    ...
    dataset_val, _ = build_dataset(
        args=args, split="val", val_fraction=args.val_fraction
    )
    ...
    if args.current_best_model:
        ...
        if args.resume:
            ...
            print(
                f"Accuracy of the best network on the {len(dataset_val)} validation images: {test_stats['acc1']:.1f}%"
            )
    ...
    if args.init_from and not args.resume:
        ...
        if utils.is_main_process():
            print(
                f"[init-from] warm-started {len(matched)}/{len(model_sd)} tensors "
                f"from {args.init_from} "
                f"(shape-mismatch: {len(skipped_shape)}, "
                f"not-in-model: {len(skipped_missing)}, "
                f"not-in-ckpt: {len(model_sd) - len(matched) - len(skipped_shape)})"
            )
    ...
    if args.teacher_model:
        ...
        teacher_config = ViTConfig.from_pretrained(args.teacher_model)
        teacher_model = ViTForImageClassification(teacher_config)
        if args.teacher_model_file:
            checkpoint = torch.load(
                args.teacher_model_file, map_location="cpu", weights_only=False
            )
    ...
    if args.resume:
        ...
        test_stats = evaluate(data_loader_val, model, device)
        print(
            f"Accuracy of the checkpoint network on the {len(dataset_val)} validation images: {test_stats['acc1']:.1f}%"
        )
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.regularization_loss:
            ...
            # )
            #
            train_stats = train_one_epoch_L1_fixed(
                model,
                teacher_model,
                criterion,
                criterion2,
                ...
                mixup_fn,
                writer=writer,
                total_epochs=args.epochs,  # ← add
                lambda_distill=0.8,  # ← add; or expose as an arg
            )
        else:
        ...
        if do_eval:
            test_stats = evaluate(data_loader_val, model, device)
            print(
                f"Accuracy of the network on the {len(dataset_val)} validation images: {test_stats['acc1']:.1f}%"
            )
