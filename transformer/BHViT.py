...
class Token_for_Attention(nn.Module):
    def __init__(self, dim, config, window_size=7):
       ...
       self.merge_avg = nn.AvgPool2d(kernel_size=window_size, stride=window_size, padding=padding)
       self.merge_max = nn.MaxPool2d(kernel_size=window_size, stride=window_size, padding=padding)
...
class BHViTSelfAttention(nn.Module):
    def __init__(self, config: ViTConfig, layer_num) -> None:
        ...
        self.token_FA = Token_for_Attention(dim=config.hidden_size[config.stages[layer_num]],config=config,window_size=config.window_size)
    ...
    def token_split(self,x,split_dim, H, W, B):
        x1,x2=torch.split(x, [split_dim, x.shape[1]-split_dim], dim=1)
...
class BinaryPatchEmbed(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        return (hidden_states + self.position_embeddings).permute(0, 2, 1).reshape(B, C2, self.H//2, self.W//2)
################################################################################################
# class BinaryPatchEmbed(nn.Module):
#     """ Image to Patch Embedding
#     """
...
class PatchEmbed(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        return (hidden_states + self.position_embeddings).permute(0, 2, 1).reshape(B, self.out_dim, self.H//2, self.W//2)
...
class BHViTEncoder(nn.Module):
    def __init__(self, config: ViTConfig) -> None:
        ...
        s0 = config.image_size // config.patch_size  # e.g. 224//4=56 or 32//4=8
        if config.some_fp:
            self.patch_embed1 = PatchEmbed(s0,      in_dim=config.hidden_size[0], out_dim=config.hidden_size[1], config=config)
            self.patch_embed2 = PatchEmbed(s0 // 2, in_dim=config.hidden_size[1], out_dim=config.hidden_size[2], config=config)
            self.patch_embed3 = PatchEmbed(s0 // 4, in_dim=config.hidden_size[2], out_dim=config.hidden_size[3], config=config)
        else:
            self.patch_embed1 = BinaryPatchEmbed(s0,      in_dim=config.hidden_size[0], out_dim=config.hidden_size[1], config=config)
            self.patch_embed2 = BinaryPatchEmbed(s0 // 2, in_dim=config.hidden_size[1], out_dim=config.hidden_size[2], config=config)
            self.patch_embed3 = BinaryPatchEmbed(s0 // 4, in_dim=config.hidden_size[2], out_dim=config.hidden_size[3], config=config)
