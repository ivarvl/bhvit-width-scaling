...
def get_model(args, model_config, model_type, weight_bits, input_bits):
    ...
    # BHViT uses these as per-stage lists, but ViTConfig strictly validates them as ints.
    # Extract them first, construct the config, then restore via object.__setattr__ to
    # bypass the strict HuggingFace dataclass setter.
    list_fields = {k: config_dict.pop(k) for k in ("hidden_size", "intermediate_size", "num_attention_heads") if k in config_dict}
